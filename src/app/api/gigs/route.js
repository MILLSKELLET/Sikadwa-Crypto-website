...
export async function POST(req) {
  try {
    ...
    const { title, description, currency, price, quantityAvailable, type } = await req.json();
...
export async function GET() {
  try {
    const gigs = await prisma.gig.findMany({ where: { status: "active" }, include: { User: true } });
